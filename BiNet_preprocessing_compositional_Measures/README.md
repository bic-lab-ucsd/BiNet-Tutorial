...
- Run [`code/reproduce_jefwan37_measures.R`](code/reproduce_jefwan37_measures.R) to link the source files, reconstruct the tidy alter table, and reproduce the one-row output.
...
The runnable merge example reads `data/jefwan37_lhq.csv`, `data/jefwan37_raw_flags.csv`, and `data/jefwan37_alter_edges.csv`. The tutorial also shows how to apply the same steps to a folder of raw Network Canvas ego, alter, and tie exports.
