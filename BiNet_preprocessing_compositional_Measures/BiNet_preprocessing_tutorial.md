...
## 1.2 Choose one LHQ and network-data route

Run either Option A or Option B in a given R session. Both options create the same three linked objects used from Section 2 onward: `egoData_linked`, `alterData_linked`, and `edgelist_linked`.

### Option A: Link your own Network Canvas and LHQ data
...
stopifnot(
  ...
  !anyNA(alterData_linked$ego_id),
  !anyNA(edgelist_linked$ego_id),
...
### Option B: Load the supplied `jefwan37` example inputs

Use this option instead of Option A when reproducing the tutorial. The repository already supplies three flattened, deidentified example inputs in the `data/` folder. This block loads the LHQ once and creates the same three linked objects as Option A.
...
The code below uses explicit `dplyr::` prefixes for grouped calculations and joins. This prevents `plyr::summarise()` or another package from masking the dplyr function, which can silently drop the grouped `ego_id` column and cause the join in Section 5.4 to fail.
...
```r
standardize_measure_id <- function(data, label) {
  if (!"ego_id" %in% names(data) && "participant_id" %in% names(data)) {
    data <- dplyr::rename(data, ego_id = participant_id)
  }
  if (!"ego_id" %in% names(data)) {
    stop(label, " is missing ego_id. Rerun its calculation in Sections 5.1–5.3.")
  }
  dplyr::mutate(data, ego_id = as.character(.data$ego_id))
}

cs_measures <- standardize_measure_id(cs_measures, "cs_measures")
mandarin_measures <- standardize_measure_id(mandarin_measures, "mandarin_measures")
homophily <- standardize_measure_id(homophily, "homophily")

ego_compositional <- cs_measures |>
  dplyr::left_join(mandarin_measures, by = "ego_id") |>
  dplyr::left_join(homophily, by = "ego_id")
