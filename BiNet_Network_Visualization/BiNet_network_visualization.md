...
The visualization loads the two node- and edge-level CSV files written by
the preprocessing tutorial: `jefwan37_tidy_alter.csv` and
`jefwan37_alter_edges.csv`. It does not load
`jefwan37_ego_compositional_wide.csv`, which contains one row of ego-level
summary measures and therefore cannot supply the nodes and ties required
for a network plot.
